...
###  run this bio400_no0, instead run the rest of the code with bio400
...
bio400_hdv2 <- left_join(bio400, headers_400, by = "sample") %>% 
...
view(bio400_hd)
## Save bio400_hdv2, then move it to  the 100x June 2022 folder so I can easily combine it with this
## data frame to get the total biomass per volume, 100x + 400x
save(bio400_hdv2, file = "bio400_hdv2.Rdata")
save(bio400_hd, file = "/Users/allisonadams/Documents/Thesis/Microplankton/R Work/100x June 2022/bio400_hd.Rdata")
#names(bio400_hd)
write_xlsx(bio400_hdv2, "bio400_hdv2_test.xlsx")
...
### Create file that removes rows with zero counts
bio400_hdv2_no0 <- bio400_hdv2 %>% 
  rowwise() %>% 
  filter(counts != 0)
write_xlsx(bio400_hdv2_no0, "bio400_hdv2__no0_test.xlsx")
