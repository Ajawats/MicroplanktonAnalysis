####################################################################
######## WIM'S GRAPH AND PALETTE FUNCTIONS AND SOME OTHERS #########
####################################################################
...
exList <- function(xlist)
{
  # assigns all components of a list to their names
  # (i.e., extracts all components)
  if (!is.list(xlist)) stop ("Input is not a list\n")
  nms <- names(xlist)
  if (is.null(nms)) names(xlist) <- paste0("xlist.", 1:length(xlist))
  for (i in 1:length(xlist))
    assign(nms[i], xlist[[i]],  envir = .GlobalEnv)
}

# exList(list(a=data.frame(a=1:4, b=4:1), b=LETTERS[1:5]))
