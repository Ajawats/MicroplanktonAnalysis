...
### 2/28/23
### Updated 3/27/23
###   !!!!! IMPORTANT NOTE !!!! DO NOT use these data until you fix the
##  data frames below. You need to update them to the ones that have the
##  chain diatoms removed from the pennate category.

### Uses a new data frame that groups the organisms by sizes small and large
##  Has code for plots
...
# above two dfs were created in the R file 03_calcs_CrFr_bySizeSmLg.R
# FrGrps.Rdata is the one that has the group_size column and biomass
# in ug per L, and is the one that should be used.
...
p <- ggplot(FrCpmOverall, aes(x=group_size, y=FrCpmAllEvents))+
 ...
 # scale_y_continuous()+
  wimGraph()+
  ggtitle("Ingestion Rates, Cells, All Sampling Events")+
...
### Dot plot Biomass
p <- ggplot(FrBugAllMn, aes(x=group_size, y=FrBugAllMn))+
  geom_point(stat = "identity",  color = "blue4", size = 5)+
  ...
  # scale_y_continuous()+
  wimGraph()+
  ggtitle("Ingestion Rates, Biomass, All Sampling Events")+
  theme(plot.title = element_text(face = "bold", size = (20)),
        axis.text.x = element_text(angle = 60, hjust = 0.8, vjust = 0.8, size = 12),
        strip.text.x = element_text(size = 14))
#xlab("Taxa Groups with Sizes") +
...
Break      <- 200
rescale    <- 200
ylabels    <- c(-60, 0, 150, 600, 1300, 4000)
brks       <- scaleBreak(ylabels, Break, rescale)

FrCpmOverall$ySquish <- scaleBreak(FrCpmOverall$FrCpmAllEvents, 
                                   Break=Break, rescale=200)

a <- ggplot(data=FrCpmOverall, aes(group_size, ySquish)) +
  geom_point(aes(color = FrCpmAllEvents>0), size = 4)+
...
FrAllOverall_ug$ySquish <- scaleBreak(FrAllOverall_ug$FrBpm_ug, 
...
### Try after reading a tutorial
### Biomass
### But per Wim's advice, don't do Biomass with log scale, b/c of the problems I already encountered
##  The log scale drops the negative values.
##  Plot needs to be done on a linear scale, with possible scale breaks
p <- ggplot(FrGrps, aes(x=group_size, FRmnBpm))+
  geom_bar(stat = "identity", fill = "darkseagreen3")+
  scale_x_discrete ("") +
  scale_y_continuous(trans = "log10") +#, breaks = c(2, 10, 25, 100, 200, 500, 2500, 10000))+
  theme(axis.text.x = element_text(angle = 60, hjust = 0.8, vjust = 0.8, size = 12),
  ...
  facet_wrap(~ event, ncol= 2, scales="free") +
  xlab("Taxa Groups with Sizes") +
  ylab("Ingestion Rate, pgC per copepod per day")+
  wimGraph()
p
 ...
 ### Try to get the <0 in a different color: Didn't work

p <- ggplot(FrGrps, aes(x=group_size, FRmnBpm))+
   geom_bar(stat = "identity", aes(fill = FRmnBpm <= 0))+
  scale_colour_manual(name = "FRmnBpm", values = c('TRUE' = "cyan", 'FALSE' = "darkseagreen3"))+
   scale_x_discrete ("") +
   scale_y_continuous(trans = "log10") +#, breaks = c(2, 10, 25, 100, 200, 500, 2500, 10000))+
   theme(axis.text.x = element_text(angle = 60, hjust = 0.8, vjust = 0.8, size = 12),
         strip.text.x = element_text(size = 14))+
