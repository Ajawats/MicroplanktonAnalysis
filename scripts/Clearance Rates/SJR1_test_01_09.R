...
taxaCenSJR1 <- filter(taxaCenSJR1, !str_detect(exp, "site|IC"))

### 1/18/23 Trry a new thing using spread-- put the FC and T24
##  counts per ml side by side
taxaCenSJR1Sprd <- taxaCenSJR1 %>% 
  spread(exp, counts_per_ml)
taxaCenSJR1Sprd <- taxaCenSJR1Sprd %>% 
  select(samp_ev, rep, grp_sz,
                          Ccpm = FC,
                          Ecpm = T24)
### Then join it with the control means
cenSJR1SprdJn <- left_join(taxaCenSJR1Sprd, taxaCenSJR1Cmn, by ="grp_sz")
cenSJR1SprdJn <- cenSJR1SprdJn %>% 
  select(samp_ev= samp_ev.x,
         grp_sz,Ccpm, Ecpm,
         Cmn)
write_xlsx(cenSJR1SprdJn, "data/Clearance Rates/cenSJR1SprdJn.xlsx")
...
### Controls
taxaCenSJR1C <- 	taxaCenSJR1[(taxaCenSJR1$exp== "FC"),]
write_xlsx(taxaCenSJR1C, "data/Clearance Rates/taxaCenSJR1C.xlsx")

### Experimentals
taxaCenSJR1E <- 	taxaCenSJR1[(taxaCenSJR1$exp== "T24"),]
save(taxaCenSJR1E, file = "data/Clearance Rates/taxaCenSJR1E.Rdata")
write_xlsx(taxaCenSJR1E, "data/Clearance Rates/taxaCenSJR1E.xlsx")

### Calculate the  mean counts of the controls  
##  "Cmn" in the mutate function indicates the Control (formerly FC) mean
taxaCenSJR1Cmn <- 	group_by(taxaCenSJR1C, grp_sz, exp) %>% 
  mutate(Cmn = mean(counts_per_ml)) %>% 
  ungroup

### Join the Controls counts per ml to the Controls means
taxaCenSJR1CmnC <- 	left_join(taxaCenSJR1C, taxaCenSJR1Cmn, by = "grp_sz")

names(taxaCenSJR1CmnC)
### Rename and re-order columns
taxaCenSJR1CmnC <- select(taxaCenSJR1CmnC, 
                        event = samp_ev.x,
                        sample = exp.x,
                        rep = rep.x,
                        group = grp_sz,
                        cpm = counts_per_ml.x,
                        Cmn)
write_xlsx(taxaCenSJR1CmnC, "data/Clearance Rates/taxaCenSJR1CmnC.xlsx")

### Remove the rep column and then remove duplicate rows
taxaCenSJR1Cmn2 <- 	select(taxaCenSJR1Cmn, -rep, -counts_per_ml)

## looks at what rows are duplicated
duplicated(taxaCenSJR1Cmn2)  

## removes duplicated rows
taxaCenSJR1Cmn2 <- taxaCenSJR1Cmn2 [!duplicated(taxaCenSJR1Cmn2), ]

### Join taxaCenSRJ1E and taxaCenSRJ1Cmn2

taxaCenSJR1CE <- 	left_join(taxaCenSJR1E, taxaCenSJR1Cmn2, by = "grp_sz")
...
sjr1Cr_a <- rowwise(taxaCenSJR1CE) %>% 
