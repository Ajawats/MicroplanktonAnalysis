...
```{r}
vol_all2 <- rowwise(raw_vol_no0) %>% 
  mutate(volume = vol_func(diameter = sa, height = la, width = wi, shape = shp, counts = counts))
```
The error is because grepl() matches that dinoflagellate row with both "ellipsoid + 2 cones + cylinder" and "ellips" becasue it finds "ellips" nside the longer shape name of "ellipsoid + 2 cones + cylinder". And the error arises becasue "ellips" requires the width dimension whereas the other one does not. Fix this by renaming the ellips shape name to a pattern that is not  also found in the other shape (i.e., ellipsoid). For example, rename those shapes to “ellips simple” or something like that:
This line of code where you are preparing the data should do the trick. Just replace “Raw_Vol” with whatever you named your dataframe in R
Raw_Vol <- mutate(Raw_Vol, shp = ifelse(shp == "ellips", "ellips simple", shp))
Then, make sure to change the search pattern in your volumes function to “ellips simple” too:
if(grepl("ellips simple", shape, ignore.case = TRUE)){
    volume = (pi/6) * diameter * height * width
  }
  
  
  I also needed to change the cone and cones2, since that would present the same error, but it wouldn't be apparent since both equations require only diameter and height. So, I tried to do both in one line and got errors: ol_all <- mutate(raw_vol_no0, shp = ifelse(shp == "ellips", "ellips1", shp) +  shp = ifelse(shp == "cone", "cone1", shp))
Error: unexpected '=' in:
"vol_all <- mutate(raw_vol_no0, shp = ifelse(shp == "ellips", "ellips1", shp) + shp ="
                    
I needed to combine them with a comma instead of a +

Luka also suggested an alternative code: vol_all <- mutate(raw_vol_no0, shp = case_when(shp == "ellips" ~ "ellips1", shp == "cone" ~ "cone1", TRUE)

```{r}
rawct_vol <- mutate(raw_vol_no0, shp = ifelse(shp == "ellips", "ellips1", shp),
                               shp = ifelse(shp == "cone", "cone1", shp))
```
 Now run the volume with this improved file
 
```{r}
vol_all <- rowwise(rawct_vol) %>% 
  mutate(volume = vol_func(diameter = sa, height = la, width = wi, shape = shp, counts = counts))
```
 
 
 Below is where I did it by using two lines of code and two different file names, but the above is better.
```{r}
#vol_all <- mutate(raw_vol_no0, shp = ifelse(shp == "ellips", "ellips1", shp))

#vol_all1 <- mutate(vol_all, shp =  ifelse(shp == "cone", "cone1", shp))
```

Add the biomass calculations   
```{r}
vol_all_bio <- rowwise(vol_all) %>% 
