...
---
Code for converting the volumes to biomass for the different organisms. See excel file, DiffDimAll_Detailed_Final_06_09, for the equations that go with the various shapes.

This version uses the aloricate ciliate equation from Menden_Deuer & Lessard 2000 (MDL).
Need to do another one with the Putt & Stoecker 1989 ratio.
Might also try with the MDL protist plankton equation
Need to separate diatoms by size and do two equations according to MDL. Here I used the equation for diatoms > 3000 µm^3, since the first dataset I'm working with is the 100x samples
```{r}
