############################################################################
####################### CLEARANCE RATE BY ESD #############################
############################################################################

### 5/25/23 using 03_calcs_CR_FR.R as the base, this script is for calculating
##  clearance rate by size, using esd, and not with the organisms already 
##  in their small and large groupings.
## This is based on a meeting with Wim on 5/24/23. See notes in 
## /Users/allisonadams/Documents/Thesis/Microplankton/Things I need access to more easily/Thesis Project and Analysis Notes.docx
##  

### Notes:
##  CR = V/T * (ln(mean control samples cpm) - ln(experimental samples cpm))/n
...
## For the top three taxa groups, ciliates, centric diatoms, flagellates.
## Note flagellate types include other, colonial, cryptomonas, euglenid, 
##  synura, in a cone

baseEsd <- volbio_all_cr %>% 
  select(samp_ev, exp, rep, Group, type, esd, cpm) %>% 
  filter(Group %in%  
           c("ciliate", "centricDiatom", "flagellate"))
save(baseEsd, file = "data/Clearance Rates/base.Rdata")

baseEsdFlag <- baseEsd %>% 
  filter(Group == "flagellate")
baseEsdCendia <- baseEsd %>% 
  filter(Group =="centricDiatom")
baseEsdCil <- baseEsd %>% 
...
baseEsdCpmE_Cmn <- 	left_join(baseEsd_E, baseEsd_Cmn, 
                          by = c("samp_ev", "Group", "esd"))
### Note: I get this error message, but
...
## is another column
names(baseEsdCpmE_Cmn)
baseEsdCpmE_Cmn <- select(baseEsdCpmE_Cmn, 
                      event = samp_ev, exp, rep,
                      Group, type = type.x, esd, cpmE,
                      CmnCpm)
...
source("scripts/01_function_clearanceRates.R")
EsdCpm_cr <- rowwise(baseEsdCpmE_Cmn) %>% 
...
#write_xlsx(EsdCpm_cr, "data/Clearance Rates/EsdCpm_cr.xlsx")
#load("data/Clearance Rates/EsdCpm_cr.Rdata")

###  Take the mean of the CR
EsdCpm_crmn <- EsdCpm_cr %>% 
  group_by(event, Group, esd) %>% 
...
EsdCpm_crmn_Cil <- EsdCpm_crmn %>% 
  filter(Group == "ciliate")

EsdCpm_crmn_CenDia <- EsdCpm_crmn %>% 
  filter(Group == "centricDiatom")

EsdCpm_crmn_Flag <- EsdCpm_crmn %>% 
  filter(Group == "flagellate")

# This is where I stopped 5/25/23
######################################################

#____________________________________________________
### Do pennate diatoms as well, and maybe dinoflagellates, for fun!
#____________________________________________________

baseEsd2 <- volbio_all_cr %>% 
  select(samp_ev, exp, rep, Group, type, esd, cpm) %>% 
  filter(Group %in%  
           c("pennateDiatom", "dinoflagellate"))
save(baseEsd2, file = "data/Clearance Rates/base.Rdata")

baseEsd2PenDia <- baseEsd2 %>% 
  filter(Group == "pennateDiatom")
baseEsd2Dino <- baseEsd2 %>% 
...
## Create the base data frame that has only the control and experimental cpm
baseEsd2_CE <- baseEsd2 %>%
  filter( str_detect(exp, "C|E"))

### Create another df from the above, with only the control samples
baseEsd2_C <- baseEsd2_CE%>% 
  filter(exp == "C") %>% 
  rename(cpmC = cpm)

### Create a df with only experimental samples
baseEsd2_E <- baseEsd2_CE%>% 
  filter(exp == "E")%>% 
  rename(cpmE = cpm) 
...
##  necessarily include the rep column, since we need the experimental samples
## individual replicate counts or biomass for the calculation.
baseEsd2CpmE_Cmn <- 	left_join(baseEsd2_E, baseEsd2_Cmn, 
                              by = c("samp_ev", "Group", "esd"))

### Remove unneeded columns, and rename and re-order remaining columns
##  Do I need the exp column?--Can't keep it since E is one column and C
## is another column
names(baseEsd2CpmE_Cmn)
baseEsd2CpmE_Cmn <- select(baseEsd2CpmE_Cmn, 
                          event = samp_ev, exp = exp.x, rep,
                          Group, type = type.x, esd, cpmE,
                          CmnCpm)
...
## since the CR was calculated for each replicate.
source("scripts/01_function_clearanceRates.R")
EsdCpm_cr <- rowwise(baseEsd2CpmE_Cmn) %>% 
  mutate(CRmlcd = cr_func(controlMnCt = CmnCpm, expCt = cpmE))
...
# This is where I stopped 5/30/23
######################################################
